...
// InsertReservation inserts a reservatio ninto the database
...
// AllRooms gets all rooms from the database 
...
// GetResrictionsForRoomByDate returns restrictions for a room by date range 
func (m *postgresDbRepo) GetResrictionsForRoomByDate(roomID int, start, end time.Time) ([]models.RoomRestriction, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3 * time.Second)
	defer cancel()
